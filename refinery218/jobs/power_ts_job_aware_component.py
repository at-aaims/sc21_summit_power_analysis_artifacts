...
#SBATCH -t 8:00:00
#SBATCH -A gen150
#SBATCH -o ../logs/power_ts_job_aware_component-%J.out
import os
import sys
import time
import glob
...
COMPUTE_SCRIPT = "power_ts_job_aware_component.py"

start_time = datetime.now()

# Source dataset base location
SOURCE_DIR='/gpfs/alpine/stf218/proj-shared/data/lake/summit_power_temp_openbmc/10s_agg'
DEST_DIR='/gpfs/alpine/stf218/proj-shared/data/lake/summit_power_temp_openbmc/power_ts_job_aware_10s_components'
...
def compute_partition(df, date_key):
    ...
    start_of_day = datetime.strptime(f'{date_key} 00:00:00+00:00',"%Y%m%d %H:%M:%S%z")

    df.reset_index(inplace=True)
    if 'timestamp' not in df.columns:
        logger.info("Time stamp column not available \n ss{df.head()}")

    df = df.loc[:,['timestamp', 'hostname', 'p0_power.mean', 'p1_power.mean', 'p0_gpu0_power.mean',
                   'p0_gpu1_power.mean','p0_gpu2_power.mean', 'p1_gpu0_power.mean', 'p1_gpu1_power.mean', 
                   'p1_gpu2_power.mean']]
